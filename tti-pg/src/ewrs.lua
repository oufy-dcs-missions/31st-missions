...
ewrs.messageDisplayTime = 30 --How long EWRS BRA messages will show for (seconds)
...
ewrs.defaultReference = "bulls" --The default reference for BRA calls - can be changed via f10 radio menu if ewrs.restrictToOneReference is false (self or bulls)
	...
	local status, result = pcall(function()
		for i = 1, #ewrs.activePlayers do
			...
			if ewrs.builtTaskingMenus[stringGroupID] == nil then
				...
				intelmenu = missionCommands.addSubMenuForGroup(groupID,"Ground Mission Location Intel Menu", mainMenu)
