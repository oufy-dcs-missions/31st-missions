...
SEF_OFFLIMITSAIRBASESFORBLUE 	= { AIRBASE.Caucasus.Anapa_Vityazevo }
...
FinalAirbaseSelectionList = {}				-- List filtered against 'off limits' airbases such as Anapa
...
function SEF_REDGENERAL_CHOOSEDEPARTUREAIRBASE()
	...
	if RedDepartureAirbase ~= nil then	
		--trigger.action.outText(RussianAirforceGeneralName.." Is Choosing "..RedDepartureAirbase.." As The Origin", 15)
	else
		RedDepartureAirbase = AIRBASE.Caucasus.Anapa_Vityazevo
		--trigger.action.outText(RussianAirforceGeneralName.." Is Choosing "..RedDepartureAirbase.." As The Origin", 15)
	end
	...
	elseif ( Group.getByName(VVSSU25TGROUPNAME) or Group.getByName(SYAAFSU24MGROUPNAME) or Group.getByName(VVSTU95MSGROUPNAME) or Group.getByName(VVSTU160GROUPNAME) or Group.getByName(VVSTU22M3GROUPNAME) ) then
		...
		if ( AttackUndefended ~= nil and ( AttackDefended ~= nil or AttackDefended == nil) ) then	
			...
			if ( AttackUndefended ~= nil and Origin ~= nil ) then
				--trigger.action.outText(RussianAirforceGeneralName.." Is Selecting "..AttackUndefended.." As The Target Airbase And "..Origin.." As The Departure Airbase For A C-130 Mission", 60)
				trigger.action.outText("Intelligence Reports Indicate "..RussianAirforceGeneralName.." Is Mobilising Aircraft", 60)
				SEF_SYAAFAN26BSPAWN(Origin, AttackUndefended)				
			elseif ( AttackUndefended ~= nil and Origin == nil ) then
				--trigger.action.outText(RussianAirforceGeneralName..": No available options for an attack origin on C-130 misson, passing this round",60)				
			elseif ( AttackUndefended == nil and Origin ~= nil ) then
				--trigger.action.outText(RussianAirforceGeneralName..": No available options for an attack destination on C-130 misson, passing this round",60)				
			else
				--trigger.action.outText(RussianAirforceGeneralName..": No options for an attack destination or an attack origin on C-130 misson, passing this round",60)				
			end
		elseif ( AttackUndefended == nil and AttackDefended ~= nil ) then
			...
			elseif ( AttackDefended ~= nil and Origin == nil ) then
				--trigger.action.outText(RussianAirforceGeneralName..": No available options for an attack origin on strike mission, passing this round",60)				
			elseif ( AttackDefended == nil and Origin ~= nil ) then
				--trigger.action.outText(RussianAirforceGeneralName..": No available options for an attack destination on strike mission, passing this round",60)				
			else
				--trigger.action.outText(RussianAirforceGeneralName..": No options for an attack destination or an attack origin on strike mission, passing this round",60)				
			end
		else
			--trigger.action.outText(RussianAirforceGeneralName..": No options for an C-130 mission, or strike mission, passing this round",60)			
		end				
	end	
...
function SEF_USAFB1BSPAWN(DepartureAirbaseName, DestinationAirbaseName)
	...
	if TargetZone.ScanData then
		for ObjectID, UnitObject in pairs( TargetZone.ScanData.Units ) do
			...
			if UnitObject:isExist() then
				local FoundUnit = UNIT:FindByName( UnitObject:getName() )
				local FoundUnitName = UnitObject:getName() 
				if ( FoundUnit and string.find(FoundUnitName, "Russian APC Tigr 233036") ) then					
	...
	USAFB1B = SPAWN
		...
		:OnSpawnGroup(
			function( SpawnGroup )						
				...
				local WP0X = DepartureZoneVec2.x
				local WP0Y = DepartureZoneVec2.y
				local WP1X = DepartureZoneVec2.x - 18520			--10 nautical miles South of Departure
				local WP1Y = DepartureZoneVec2.y + 37040 			--20 nautical miles East of Departure
	...
	:SpawnAtParkingSpot(AIRBASE:FindByName(AIRBASE.Caucasus.Kutaisi), {3}, SPAWN.Takeoff.Hot)
end
	...
	USAFB52H = SPAWN
		...
		:OnSpawnGroup(
			function( SpawnGroup )						
				...
				local WP0X = DepartureZoneVec2.x
				local WP0Y = DepartureZoneVec2.y
				local WP1X = DepartureZoneVec2.x - 18520			--10 nautical miles South of Al Dhafra
				local WP1Y = DepartureZoneVec2.y + 37040 			--20 nautical miles East of Al Dhafra					
	...
	if ( Group.getByName(VVSSU25TGROUPNAME) or Group.getByName(SYAAFSU24MGROUPNAME) or Group.getByName(VVSTU95MSGROUPNAME) or Group.getByName(VVSTU160GROUPNAME) or Group.getByName(VVSTU22M3GROUPNAME) ) then
		...
		elseif ( ChooseAttackElements > 50 and ChooseAttackElements <= 75 ) then
			...
			if ( ChooseBomberElements <= 30 ) then			
				SEF_VVSTU95MS( AIRBASE.Caucasus.Anapa_Vityazevo, AttackDefended )				
			elseif ( ChooseBomberElements > 30 and ChooseBomberElements <= 60 ) then			
				SEF_VVSTU160( AIRBASE.Caucasus.Anapa_Vityazevo, AttackDefended )				
			elseif ( ChooseBomberElements > 60 and ChooseBomberElements <= 90 ) then			
				SEF_VVSTU22M3( AIRBASE.Caucasus.Anapa_Vityazevo, AttackDefended )				
			else						
			end			
		else
	...
	if ( AutomatedBlueStrikes == 1 ) then
		if ( Group.getByName(USAFB1BGROUPNAME) or Group.getByName(USAFB52HGROUPNAME) ) then
			...
			if ( ChooseBomberElements <= 50 ) then
				SEF_USAFB1BSPAWN( AIRBASE.Caucasus.Kutaisi, AttackDefended )			
			else
				SEF_USAFB52HSPAWN( AIRBASE.Caucasus.Kutaisi, AttackDefended )			
			end						
		end	
	end	
end
...
--SEF_CAPAIRBASE(AIRBASE.Caucasus.Beslan, 2)
	...
	if Event.id == world.event.S_EVENT_BASE_CAPTURED then
		if Event.initiator then	
			...
			if ( CapturedBaseCapturingUnitCoalition == 1 and timer.getAbsTime() >= CaptureMessageLockout + 5 ) then
				trigger.action.outText(CapturedBaseName.." Has Been Captured By The Russians", 15)
						...
						if ( DeadUnitObjectCategory == 1 ) then 										-- UNIT
							if ( DeadUnitCategory == 2 or DeadUnitCategory == 3 ) then 					-- GROUND_UNIT or SHIP
								if ( string.find(DeadUnitName, "Russian APC Tigr 233036") ) then  
									...
									ClearFieldMkIIUnitInterment[UnitIntermentTableLength] = DeadUnitName
								end	
							else
							end
						elseif ( DeadUnitObjectCategory == 3 ) then 									-- STATIC
							StaticIntermentTableLength = StaticIntermentTableLength + 1			
							ClearFieldMkIIStaticInterment[StaticIntermentTableLength] = DeadUnitName												
						else
						end
					else
					end
