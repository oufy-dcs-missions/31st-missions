...
if enable_BAI_tasks == true then
    ...
    if enable_BAI_HARD_tasks == true then
        function MakeTaskBAIHARD()
            ...
            BAI_HARD_task =
                SPAWN:NewWithAlias(TemplateBAIHARD[tempRandGroup4], tempGrpAlias4):OnSpawnGroup(
                function(SpawnGroup)
    ...
    --↓↓ ANTISHIP TASK ↓↓-----------------------------------------------------------------------------------------------------------------------------------------------
    if enable_AntiShip_tasks == true then
        function MakeTaskNavBAI()
            local RngNum = randomnumber()
            local Rng_OP_NAME3 = RandOpName()
            local Rng_OP_Brief_NAVBAI = MissionTextNAV()
            ...
            NavBAI_task =
                SPAWN:NewWithAlias(TemplateNavBAI[tempRandGroup3], tempGrpAlias3):OnSpawnGroup(
                function(SpawnGroup)
                    NAVBAI_Marker_Loc = SpawnGroup:GetCoordinate()
                    NAVBAI_MAPMARKER =
                        MARKER:New(
                        NAVBAI_Marker_Loc,
                        '⇨ Op: ' ..
                            Rng_OP_NAME3 ..
                                ' ⇦ ☾ NAV Strike ☽ :' ..
                                    '\n' ..
                                        NAVBAI_Marker_Loc:ToStringLLDDM(SETTINGS:SetLL_Accuracy()) ..
                                            '\n' .. NAVBAI_Marker_Loc:ToStringLLDMS(SETTINGS:SetLL_Accuracy())
                    ):ToAll():ReadOnly()
    
                    function BAI_NAV_INFO_MENU(SpawnGroup)
                        NAVBAI_HARD_INFO =
                            trigger.action.outText(
                            '⇨ Operation: ' ..
                                Rng_OP_NAME3 ..
                                                ...
                                                '\n \n' ..
                                                    NAVBAI_Marker_Loc:ToStringLLDDM(SETTINGS:SetLL_Accuracy()) ..
                                                        '\n' ..
                                                            NAVBAI_Marker_Loc:ToStringLLDMS(SETTINGS:SetLL_Accuracy()) ..
                                                                '\n' ..
                                                                    NAVBAI_Marker_Loc:ToStringMGRS(
                                                                        SETTINGS:SetLL_Accuracy()
                                                                    ),
                            60
                        )
                    ...
                    MenuCoalition_BLUE_TASK_NAVBAI =
                        MENU_COALITION_COMMAND:New(
                        coalition.side.BLUE,
                        '⇨ ' .. Rng_OP_NAME3 .. ' ⇦ ☾ NAV Strike ☽',
                        MenuCoalition_BLUE_TASK,
                        BAI_NAV_INFO_MENU
                    )
                    ...
                    function SpawnGroup:OnEventDead(EventData)
                        if SpawnGroup:GetSize() <= 1 then
                            trigger.action.outText('⇨ Operation: ' .. Rng_OP_NAME3 .. ' ⇦ ☾ NAV Strike ☽ SUCCESSFUL', 15)
                            NAVBAI_MAPMARKER:Remove(1)
                            SNDTAKEOFF = USERSOUND:New('TransmisionEntrante.ogg')
                            SNDTAKEOFF:ToCoalition(coalition.side.BLUE)
                            MenuCoalition_BLUE_TASK_NAVBAI:Remove()
                            ActiveTasksNAVbai = false
                        end
                    end
                end
        ...
        SCHEDULER:New(
            nil,
            function()
                if ActiveTasksNAVbai == false then
                    LastTask = MakeTaskNavBAI()
                    ActiveTasksNAVbai = true
                elseif ActiveTasksNAVbai == true then
                -- no hagas nada
                end
            end,
            {},
...
function MakeTaskBAI_menucmd()
    ...
    BAI_task =
        ...
        function(SpawnGroup)
            ...
            BAIJTAC4 = SPAWN:NewWithAlias('jtac', 'jtac  BAI-' .. RngNum):SpawnFromVec2(newlocation1)
            ...
            function SpawnGroup:OnEventDead(EventData)
                if SpawnGroup:GetSize() <= 1 then
                    ...
                    SNDTAKEOFF = USERSOUND:New('TransmisionEntrante.ogg')
                    SNDTAKEOFF:ToCoalition(coalition.side.BLUE)
                    BAIJTACName5 = BAIJTAC:GetName()
                    Group.getByName(BAIJTACName4):destroy()
                    MenuCoalition_BLUE_TASK_BAI:Remove()
                end
            end
        end
...
function MakeTaskNavBAI_menucmd()
    local RngNum = randomnumber()
    local Rng_OP_NAVBAICMD = RandOpName()
    local Rng_OP_Brief_NAVBAICMD = MissionTextNAV()
    local RNGzone2 = math.random(1, #(TemplateNavalZones))
    local tempRandGroup3 = math.random(1, #(TemplateNavBAI))
    local tempGrpAlias3 = '⚓ NAVAL ' .. RngNum
    ...
    NavBAI_task =
        ...
        function(SpawnGroup)
            ...
            function SpawnGroup:OnEventDead(EventData)
                if SpawnGroup:GetSize() <= 1 then
                    trigger.action.outText('⇨ Operation: ' .. Rng_OP_NAVBAICMD .. ' ⇦ ☾ NAV Strike ☽ SUCCESSFUL', 15)
                    NAVBAI_MAPMARKER:Remove(1)
                    SNDTAKEOFF = USERSOUND:New('TransmisionEntrante.ogg')
                    SNDTAKEOFF:ToCoalition(coalition.side.BLUE)
                    MenuCoalition_BLUE_TASK_NAVBAI:Remove()
...
):SetSpeed(Tanker_BASKET_Speed):SetHomeBase(AIRBASE.Syria.Adana_Sakirpasa):SetRadio(264.25):SetModex(512):SetTACAN(25, 'ARC'):SetRacetrackDistances(
    50,
    5
):Start()
...
):SetSpeed(Tanker_BOOM_Speed_Slow):SetHomeBase(AIRBASE.Syria.Adana_Sakirpasa):SetRadio(129):SetModex(513):SetTACAN(28, 'TEX'):SetRacetrackDistances(
        15,
        15
):Start()
...
RECOVERYTANKER:New(UNIT:FindByName('Anchor-Overlord-1'), '☮ Overlord'):SetAWACS():SetTakeoffCold():SetRadio(
        265.8
):SetAltitude(37000):SetHomeBase(AIRBASE.Syria.Adana_Sakirpasa):SetCallsign(CALLSIGN.AWACS.Overlord):SetRacetrackDistances(
        40,
        40
...
if enable_RescueHelos == true then
    local rescuehelo =
        RESCUEHELO:New(UNIT:FindByName('⚓ CVN-74 Stennis'), '☮ Rescate'):SetHomeBase(AIRBASE:FindByName('⚓ CVN-74 Stennis-3')):SetTakeoffHot(

    ):SetRespawnOn():SetRescueDuration(1):SetModex(42):Start()

    local rescueheloB =
        RESCUEHELO:New(UNIT:FindByName('⚓ LHA-1 Tarawa'), '☮ Rescate'):SetHomeBase(AIRBASE:FindByName('⚓ LHA-1 Tarawa-1')):SetTakeoffHot(
    ...
    local rescueheloC =
        ...
        AIRBASE:FindByName('⚓ CVN-73 George Washington-2')
...
if (enable_AntiShip_tasks == true) then
    trigger.action.outText('Anti Ship tasks: ✔', 20)
end
if (enable_SEAD_tasks == true) then
