...
function JTACAutoLase(jtacGroupName, laserCode, smoke, lock, colour)
    ...
    if enable_mission_editor_jtac_unit == true then
        if jtacUnit:isActive() == false then
            ...
            timer.scheduleFunction(
                timerJTACAutoLase,
                {jtacGroupName, laserCode, smoke, lock, colour},
                timer.getTime() + 30
            )
    ...
    if enemyUnit == nil and GLOBAL_JTAC_CURRENT_TARGETS[jtacGroupName] ~= nil then
        ...
        if tempUnit ~= nil and tempUnit:isActive() == true then
    ...
    if enemyUnit == nil then
        ...
        if enemyUnit ~= nil then
            ...
            GLOBAL_JTAC_CURRENT_TARGETS[jtacGroupName] = {
                name = enemyUnit:getName(),
                unitType = enemyUnit:getTypeName(),
                unitId = enemyUnit:getID()
            }
                    ...
                    '' ..
                        jtacGroupName ..
                            ' LSR: ' ..
                                laserCode ..
                                    ' - TGT: ' ..
                                        enemyUnit:getTypeName() ..
                                            getPositionString(enemyUnit) .. heightString(enemyUnit) .. '\n'
...
function latLngString(unit, acc)
    ...
    return string.format('%02d', latDeg) ..
        ' ' ..
            string.format(minFrmtStr, latMin) ..
                "'" ..
                    latHemi ..
                        '' ..
    ...
    if acc == 0 then
        ...
        return MGRS.UTMZone ..
            ' ' ..
                MGRS.MGRSDigraph ..
                    ' ' ..
                        string.format('%0' .. acc .. 'd', roundNumber(MGRS.Easting / (10 ^ (5 - acc)), 0)) ..
...
function heightString(unit)
    ...
    return string.format('\nELEV: %dm / %dft', heightMetersRounded, heightFeetRounded)
    --return string.format('\nELEV: %dft\n \n', heightFeetRounded, heightMetersRounded)
end
