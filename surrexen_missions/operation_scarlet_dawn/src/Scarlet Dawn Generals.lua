...
SEF_OFFLIMITSAIRBASESFORRED 	= { "Incirlik", "Ramat David", "Akrotiri", "Lakatamia", "Sanliurfa", "Kharab Ishk", "H3" }
							
SEF_OFFLIMITSAIRBASESFORBLUE 	= { "Deir ez-Zor" }
...
function SEF_SYAAFAN26BSPAWN(DepartureAirbaseName, DestinationAirbaseName)
	...
	if ( DepartureAirbaseName == "Taftanaz" or DepartureAirbaseName == "Kharab Ishk" ) then
	...
	elseif ( DepartureAirbaseName == "Qabr as Sitt" or DepartureAirbaseName == "Marj as Sultan North" or DepartureAirbaseName == "Marj as Sultan South" or DepartureAirbaseName == "Kiryat Shmona" or DepartureAirbaseName == "Haifa" or DepartureAirbaseName == "Rayak" or DepartureAirbaseName == "Naqoura" or DepartureAirbaseName == "Rosh Pina" or DepartureAirbaseName == "Tha'lah" or DepartureAirbaseName == "H4" ) then
	...
	SYAAFAN26B = SPAWN:NewWithAlias("Plane Template", "SyAAF An-26B"):InitRandomizeTemplate( { "SyAAF An-26B" } )
	...
	:SpawnAtAirbase( AIRBASE:FindByName( DepartureAirbaseName ), SPAWN.Takeoff.Hot, nil, AIRBASE.TerminalType.OpenBig )
...
function SEF_USAFC130SPAWN(DepartureAirbaseName, DestinationAirbaseName)
	...
	elseif ( DepartureAirbaseName == "At Tanf" ) then
		if ( Airbase.getByName("H3"):getCoalition() == 2 ) then
			DepartureAirbaseName = "H3"
		else
			DepartureAirbaseName = "Ramat David"
		end	
	else
...
function SEF_SYAAFSU24M(DepartureAirbaseName, DestinationAirbaseName)
	...
	SYAAFSU24M = SPAWN:NewWithAlias("Plane Template", "SyAAF Su-24M"):InitRandomizeTemplate( { "SyAAF Su-24M" } )
	...
	VVSTU95MS = SPAWN:NewWithAlias("Plane Template", "VVS Tu-95MS"):InitRandomizeTemplate( { "VVS Tu-95MS" } )
		...
		:OnSpawnGroup(
			function( SpawnGroup )						
				...
				local WP0X = DepartureZoneVec2.x
				local WP0Y = DepartureZoneVec2.y
				local WP1X = TargetZoneVec2.x 
				local WP1Y = TargetZoneVec2.y + 46300
				local WP2X = TargetZoneVec2.x
				local WP2Y = TargetZoneVec2.y
				local WP3X = DepartureZoneVec2.x
				local WP3Y = DepartureZoneVec2.y
...
function SEF_IRIAFMI8SPAWN(DepartureAirbaseName, DestinationAirbaseName)
	...
	IRIAFMI8 = SPAWN:NewWithAlias("Helicopter Template", "IRIAF Mi-8MTV2"):InitRandomizeTemplate( { "IRIAF Mi-8MTV2" } )	
...
function SEF_USAFAH64DSPAWN()

	USAFAH64D_DATA[1].Vec2 		= nil
	USAFAH64D_DATA[1].TimeStamp = nil
	USAFAH64D_DATA[2].Vec2 		= nil
	USAFAH64D_DATA[2].TimeStamp = nil
	...
	Apache_DepartureZone 	= AIRBASE:FindByName("London"):GetZone()
	DepartureAirbaseName = "London"
		
	Apache_WP1ZoneVec2 	= ZONE:FindByName("Samandag"):GetVec2()
	Apache_WP2ZoneVec2 	= ZONE:FindByName("Isikli"):GetVec2()
	Apache_WP3ZoneVec2 	= ZONE:FindByName("Iskenderun"):GetVec2()
	Apache_WP4ZoneVec2 	= ZONE:FindByName("Kumlu"):GetVec2()
	Apache_WP5ZoneVec2 	= AIRBASE:FindByName("London"):GetZone():GetVec2()
		
	USAFAH64D = SPAWN:NewWithAlias("Helicopter Template", "USAF AH-64D"):InitRandomizeTemplate( { "USAF AH-64D" } )	
	
		:OnSpawnGroup(
			function( SpawnGroup )						
				USAFAH64DGROUPNAME = SpawnGroup.GroupName
				USAFAH64DGROUPID = Group.getByName(USAFAH64DGROUPNAME):getID()
				USAFAH64DGROUP = GROUP:FindByName(SpawnGroup.GroupName)				
				
				local Apache_DepartureZoneVec2 = Apache_DepartureZone:GetVec2()				
				local WP0X = Apache_DepartureZoneVec2.x
				local WP0Y = Apache_DepartureZoneVec2.y
				local WP1X = Apache_WP1ZoneVec2.x
				local WP1Y = Apache_WP1ZoneVec2.y 
				local WP2X = Apache_WP2ZoneVec2.x 
				local WP2Y = Apache_WP2ZoneVec2.y 
				local WP3X = Apache_WP3ZoneVec2.x 
				local WP3Y = Apache_WP3ZoneVec2.y
				local WP4X = Apache_WP4ZoneVec2.x 
				local WP4Y = Apache_WP4ZoneVec2.y
				local WP5X = Apache_WP5ZoneVec2.x
				local WP5Y = Apache_WP5ZoneVec2.y 			
			...
			USAFAH64DGROUP:SetTask(Mission)
			end
		)
		--:SpawnInZone( SpawnZone, false, 304.8, 304.8 )
		:SpawnAtAirbase( AIRBASE:FindByName( DepartureAirbaseName ), SPAWN.Takeoff.Hot )
...
function SEF_SYAAFMI24PSPAWN()

	SYAAFMI24P_DATA[1].Vec2 		= nil
	SYAAFMI24P_DATA[1].TimeStamp 	= nil
	SYAAFMI24P_DATA[2].Vec2 		= nil
	SYAAFMI24P_DATA[2].TimeStamp 	= nil
	
	--[[		
	Launch From Taftanaz
	...
	--local DepartureZone = AIRBASE:FindByName(DepartureAirbaseName):GetZone()
	--local DestinationZone = AIRBASE:FindByName(DestinationAirbaseName):GetZone()
	...
	if (FlightSelection <=33 and Airbase.getByName("Taftanaz"):getCoalition() == 1) then	
		--North
		Hind_DepartureZone 	= AIRBASE:FindByName("Taftanaz"):GetZone()
		DepartureAirbaseName = "Taftanaz"
		
		if ( FlightDirection <= 50 ) then
			--Clockwise
			Hind_WP1ZoneVec2 	= ZONE:FindByName("Idlib"):GetVec2()
			Hind_WP2ZoneVec2 	= ZONE:FindByName("Bohsin"):GetVec2()
			Hind_WP3ZoneVec2 	= ZONE:FindByName("Avsuya"):GetVec2()
			Hind_WP4ZoneVec2 	= ZONE:FindByName("Altinozu"):GetVec2()
			Hind_WP5ZoneVec2 	= ZONE:FindByName("Karbeyaz"):GetVec2()		
		else
			--Anticlockwise		
			Hind_WP1ZoneVec2 	= ZONE:FindByName("Karbeyaz"):GetVec2()
			Hind_WP2ZoneVec2 	= ZONE:FindByName("Altinozu"):GetVec2()
			Hind_WP3ZoneVec2 	= ZONE:FindByName("Avsuya"):GetVec2()
			Hind_WP4ZoneVec2 	= ZONE:FindByName("Bohsin"):GetVec2()
			Hind_WP5ZoneVec2 	= ZONE:FindByName("Idlib"):GetVec2()
		end
	elseif (FlightSelection > 33 and FlightSelection <= 66 and Airbase.getByName("Shayrat"):getCoalition() == 1) then
		--Central
		Hind_DepartureZone 	= AIRBASE:FindByName("Shayrat"):GetZone()
		DepartureAirbaseName = "Shayrat"
		
		if ( FlightDirection <= 50 ) then
			--Clockwise
			Hind_WP1ZoneVec2 	= AIRBASE:FindByName("Hama"):GetZone():GetVec2()
			Hind_WP2ZoneVec2 	= ZONE:FindByName("Ar Rastan"):GetVec2()
			Hind_WP3ZoneVec2 	= ZONE:FindByName("Homs"):GetVec2()
			Hind_WP4ZoneVec2 	= ZONE:FindByName("Dhahabiyah"):GetVec2()
			Hind_WP5ZoneVec2 	= AIRBASE:FindByName("Al Qusayr"):GetZone():GetVec2()		
		else
			--Anticlockwise		
			Hind_WP1ZoneVec2 	= AIRBASE:FindByName("Al Qusayr"):GetZone():GetVec2()
			Hind_WP2ZoneVec2 	= ZONE:FindByName("Dhahabiyah"):GetVec2()
			Hind_WP3ZoneVec2 	= ZONE:FindByName("Homs"):GetVec2()
			Hind_WP4ZoneVec2 	= ZONE:FindByName("Ar Rastan"):GetVec2()
			Hind_WP5ZoneVec2 	= AIRBASE:FindByName("Hama"):GetZone():GetVec2()
		end
	elseif (FlightSelection > 66 and FlightSelection <= 100 and Airbase.getByName("Qabr as Sitt"):getCoalition() == 1) then
		--South
		Hind_DepartureZone 	= AIRBASE:FindByName("Qabr as Sitt"):GetZone()
		DepartureAirbaseName = "Qabr as Sitt"
		
		if ( FlightDirection <= 50 ) then
			--Clockwise
			Hind_WP1ZoneVec2 	= ZONE:FindByName("Golan Heights"):GetVec2()
			Hind_WP2ZoneVec2 	= ZONE:FindByName("Dar'a"):GetVec2()
			Hind_WP3ZoneVec2 	= ZONE:FindByName("Al Jizah"):GetVec2()
			Hind_WP4ZoneVec2 	= ZONE:FindByName("As Suwayda"):GetVec2()
			Hind_WP5ZoneVec2 	= ZONE:FindByName("As Sawara"):GetVec2()		
		else
			--Anticlockwise		
			Hind_WP1ZoneVec2 	= ZONE:FindByName("As Sawara"):GetVec2()
			Hind_WP2ZoneVec2 	= ZONE:FindByName("As Suwayda"):GetVec2()
			Hind_WP3ZoneVec2 	= ZONE:FindByName("Al Jizah"):GetVec2()
			Hind_WP4ZoneVec2 	= ZONE:FindByName("Dar'a"):GetVec2()
			Hind_WP5ZoneVec2 	= ZONE:FindByName("Golan Heights"):GetVec2()
		end	
	else
	end
	
	SYAAFMI24P = SPAWN:NewWithAlias("Helicopter Template", "SyAAF Mi-24P"):InitRandomizeTemplate( { "SyAAF Mi-24P" } )	
	
		:OnSpawnGroup(
			function( SpawnGroup )						
				SYAAFMI24PGROUPNAME = SpawnGroup.GroupName
				SYAAFMI24PGROUP = GROUP:FindByName(SpawnGroup.GroupName)				
				
				local Hind_DepartureZoneVec2 = Hind_DepartureZone:GetVec2()				
				local WP0X = Hind_DepartureZoneVec2.x
				local WP0Y = Hind_DepartureZoneVec2.y
				local WP1X = Hind_WP1ZoneVec2.x
				local WP1Y = Hind_WP1ZoneVec2.y 
				local WP2X = Hind_WP2ZoneVec2.x 
				local WP2Y = Hind_WP2ZoneVec2.y 
				local WP3X = Hind_WP3ZoneVec2.x 
				local WP3Y = Hind_WP3ZoneVec2.y
				local WP4X = Hind_WP4ZoneVec2.x 
				local WP4Y = Hind_WP4ZoneVec2.y
				local WP5X = Hind_WP5ZoneVec2.x
				local WP5Y = Hind_WP5ZoneVec2.y 				
			...
			SYAAFMI24PGROUP:SetTask(Mission)
			end
		)
		--:SpawnInZone( SpawnZone, true, 304.8, 304.8 )
		:SpawnAtAirbase( AIRBASE:FindByName( DepartureAirbaseName ), SPAWN.Takeoff.Hot )
...
function SEF_USAFB1BSPAWN(DepartureAirbaseName, DestinationAirbaseName)
	...
	if TargetZone.ScanData then
		for ObjectID, UnitObject in pairs( TargetZone.ScanData.Units ) do
			...
			if UnitObject:isExist() then
				local FoundUnit = UNIT:FindByName( UnitObject:getName() )
				local FoundUnitName = UnitObject:getName() 
				if ( FoundUnit and string.find(FoundUnitName, "Syrian APC Tigr 233036") ) then					
	...
	USAFB1B = SPAWN:NewWithAlias("Plane Template", "USAF B-1B"):InitRandomizeTemplate( { "USAF B-1B" } )
		...
		:OnSpawnGroup(
			function( SpawnGroup )						
				...
				local WP0X = DepartureZoneVec2.x
				local WP0Y = DepartureZoneVec2.y
				local WP1X = DepartureZoneVec2.x - 18520			--10 nautical miles South of Incirlik
				local WP1Y = DepartureZoneVec2.y - 37040 			--20 nautical miles West of Incirlik
				local WP2X = TargetZoneVec2.x 
				local WP2Y = TargetZoneVec2.y - 37040 				--20 nautical miles West of Target
	...
	:SpawnAtParkingSpot(AIRBASE:FindByName("Incirlik"), {71}, SPAWN.Takeoff.Hot)
end
	...
	USAFB52H = SPAWN:NewWithAlias("Plane Template", "USAF B-52H"):InitRandomizeTemplate( { "USAF B-52H" } )
		...
		:OnSpawnGroup(
			function( SpawnGroup )						
				...
				local WP1Y = DepartureZoneVec2.y - 37040 			--20 nautical miles West of Incirlik					
				local WP2X = TargetZoneVec2.x 
				local WP2Y = TargetZoneVec2.y - 46300 				--25 nautical miles West of the Target
				local WP3X = TargetZoneVec2.x
				local WP3Y = TargetZoneVec2.y
				local WP4X = DepartureZoneVec2.x
				local WP4Y = DepartureZoneVec2.y
	...
	if ( AutomatedBlueStrikes == 1 ) then
		if ( Group.getByName(USAFB1BGROUPNAME) or Group.getByName(USAFB52HGROUPNAME) ) then
			...
			if ( ChooseBomberElements <= 50 ) then
				SEF_USAFB1BSPAWN( "Incirlik", AttackDefended )			
			else
				SEF_USAFB52HSPAWN( "Incirlik", AttackDefended )			
			end						
		end	
	end	
end
...
function SEF_REDGENERAL_SCHEDULESTRIKE()	
	timer.scheduleFunction(SEF_REDGENERAL_ATTACKAIRBASE, nil, timer.getTime() + math.random(RedStrikeFrequencyMin, RedStrikeFrequencyMax))	
end

function SEF_BLUEGENERAL_SCHEDULESTRIKE()	
	timer.scheduleFunction(SEF_BLUEGENERAL_ATTACKAIRBASE, nil, timer.getTime() + math.random(BlueStrikeFrequencyMin, BlueStrikeFrequencyMax))	
end
...
function SEF_RED_HELOATTACKSCHEDULER()
...
function SEF_BLUE_HELOATTACKSCHEDULER()
...
--SEF_CAPAIRBASE("Gaziantep", 1)
--SEF_USAFB1BSPAWN( "Incirlik", "Hatay" )

--////END TEST CASE


timer.scheduleFunction(SEF_REDGENERAL_ATTACKAIRBASE, nil, timer.getTime() + math.random(900, 1200))
timer.scheduleFunction(SEF_BLUEGENERAL_ATTACKAIRBASE, nil, timer.getTime() + math.random(900, 1200))
