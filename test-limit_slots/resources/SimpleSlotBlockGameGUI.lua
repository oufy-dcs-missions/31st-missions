...
--[[
   ...
   Put this file in <Saved Games>/DCS/Scripts/Hooks
   ...
   trigger.action.setUserFlag("ssb_UNIT_NAME",100)

   Where UNIT_NAME is the name of the flag you want to change and 100 the value you want to set the flag too.

   As an example, there are two aircraft units, HELI1 and HELI2 and the initial flag to turn on SSB
   ...
   This will make HELLI2 unusable until that flag is changed to 0 (assuming the ssb.enabledFlagValue is 0).
   ...
   Additions:
     ...
     => trigger.action.setUserFlag("HELI2",100) -- This will kick the unit HELI2 when in the game.
...
-- trigger.action.setUserFlag("ssb_UNIT_NAME",100)
-- where UNIT_NAME is the Unit name (not pilot name) and 100 is the value you're setting the flag too which must
...
ssb.commanderPlayerUCID = {
  "a9e033cc9e64f8b35a45ae4e8666e556", -- Gemini
  "91df27411f66d35aa76259e7074e5608", -- Marco
  "0203091d34c33abcebc35ab48e96a4f9", -- Radius
}
...
function ssb.shouldAllowAircraftSlot(_playerID, _slotID) -- _slotID == Unit ID unless its multi aircraft in which case slotID is unitId_seatID
  ...
  local _unitName = ssb.getUnitName(_slotID)
  ...
  --  return true
  --end
  if _unitName == nil or _unitName == "" then
    net.log("SSB - Unable to get unit name for slot ".._slotID)
    return true
  end

  --_groupName = ssb.trimStr(_groupName)
  --_unitName = ssb.trimStr(_unitName)

  --if not ssb.checkClanSlot(_playerID, _groupName) then
  --  return false
  --end
  if not ssb.checkClanSlot(_playerID, _unitName) then
    return false
  end

  -- check flag value
  --local _flag = ssb.getFlagValue(_groupName)
  local _flag = ssb.getFlagValue("ssb_".._unitName)
...
function ssb.allowAircraftSlot(_playerID, _slotID) -- _slotID == Unit ID unless its multi aircraft in which case slotID is unitId_seatID (added by FlightControl)
  ...
  local _result = ssb.setFlagValue("ssb_".._unitName, 0)
...
function ssb.getFlagValue(_flag)
  ...
  if not _status and _error then
    ...
    --disabled
    --net.log("SSB - getFlagValue ".._flag.." value="..tonumber(_status))
...
function ssb.setFlagValue(_flag, _number) -- Added by FlightControl
  ...
  --net.log("SSB - setFlagValue ".._flag.." value=".._number)
  return true
end
...
function ssb.getUnitName(_slotID)

  local _name = DCS.getUnitProperty(_slotID, DCS.UNIT_NAME)
...
net.log("Loaded - SIMPLE SLOT BLOCK v".. ssb.version.. " by Gemini")
